...
# 创建网络并加载样本
solver = net3.net(train_batch_size, lr, weight_decay)
solver.load_sample_and_label(np.hstack((fft, power, dps3)), train_label)
solver.load_sample_and_label_test(np.hstack((fft_test, power_test, dps3_test)), test_label)
...
for i in train_sequence:
	print '第', i, '次迭代'
	net3.layer.update_method.iteration  = i
	solver.forward()
	solver.backward()
	solver.update()
